...
if (!isset($_POST['username'], $_POST['password'], $_POST['email'])) {
    header("Location: registerPage.php?register=complete");
    exit();
}
...
if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
    header("Location: registerPage.php?register=emal");
    exit();
}
if (strlen($_POST['password']) > 20 || strlen($_POST['password']) < 5) {
    header("Location: registerPage.php?register=pwd");
    exit();
}
if ($stmt = $con->prepare('SELECT id, password FROM logindetails WHERE username = ?')) {
    ...
    $stmt->store_result();
    if ($stmt->num_rows > 0) {
        header("Location: registerPage.php?register=usrnme");
        exit();
    } else {
